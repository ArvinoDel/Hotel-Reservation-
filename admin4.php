  ...
  <section id="home" class="home-section">
    ...
    <div class="card">
       ...
       <?php
        ...
        <tr id="property">
          <?php $id_property = $result['id_property'];?>
           <th scope="row"><?php echo $result['id_property']?></th>
             <td scope="row"><?php echo $result['room']?></td>
             <td scope="row"><?php echo $result['stock']?></td>
             <td scope="row"> <img src="foto.jpeg/<?php echo $result['img1']?>" width="60px" style="border-radius: 5px;"></td>
             <td scope="row">
               <form method="post">
                 <input type="number" class="form-input" style="width: 40px;" name="stok">
                 <input type="hidden" name="id" value="<?=$id_property?>">
                 <input type="submit" class="btn btn-success" name="edit" value="edit stock">
               </form>
               <?php } ?>
            <?php
            include 'konek.php';
            if(isset($_POST['edit'])){
              $stock = $_POST['stok'];
              $id_property1 = $_POST['id'];
              $sql3 = "SELECT * from property where id_property=$id_property1";
              $q3 = mysqli_query($info, $sql3);
              $r3 = mysqli_fetch_assoc($q3);
              $stockawal= $r3['stock'];
              $stockakhir = $stock + $stockawal;
              $sql1 = "UPDATE property SET stock=$stockakhir where id_property=$id_property1";
              $result3 = mysqli_query($info, $sql1);

              if ($result3) {
                  echo '<div class="success-card position-absolute top-0 left-0" style="width: 300px; padding: 20px; background-color: #4CAF50; color: #fff; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); text-align: center;" onclick="redirectToBootstrap()">
                  Successful!
                </div>
              ';
              echo "<script>setTimeout(function(){window.location.href = 'admin4.php';}, 1000);</script>";
              } else {
                  echo "Error updating stock: " . mysqli_error($info);
              }
          }
            ?>
            </td>
